...
2) While loop (while helper method does not equal nil)
  2a) helper method will use each_char to iterate through the string and check to see if there is a duplicate character by comparing the characters in the same case
...
- Is it case sensitive?
  ...
  def helper_method(string)
    iterate through string
      compare each letter (downcased) with letter following (downcased), if the same, return index of second
    end
    return nil
  end
...
def index_of_duplicate(string)
  string.each_char.with_index do |char, idx|
    break if idx == string.length - 1
    return (idx + 1) if string[idx].downcase == string[idx + 1].downcase
  end
  nil
end
