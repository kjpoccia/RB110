=begin
Problem:
- Given a string object, return the longest sentence in the file, and how many words are in it.
- Sentences are defined as strings ending in !, ?, and .

Examples:
- Not repasted here, too long

Data structure:
- Array

Algorithm:
- Turn the entire string into an array, separate using spaces. 
- Use each with index to iterate through the array of words, if a period, ?, ! is found, store the index in another array
- Iterate through the index array and calculate the difference between each element and the element to its right
- Store the two elements with the greatest difference
- Use those two indices to return the longest sentence, as well as the word count
...
def longest_sentence(string)
  ...
  array.each_with_index do |word, idx|
    index_array << idx if word.include?("?") || word.include?("!") || word.include?(".")
  end

  start, length = 0, 0, 0
  index_array.each_with_index do |num, idx|
    if idx == index_array.length - 1
      break
    elsif (index_array[idx + 1] - index_array[idx]) > length
     # binding.pry
      length = index_array[idx + 1] - index_array[idx]
      start = index_array[idx] + 1
    end
  end
  
  word = array[start, length].join(" ")
  words = array[start, length].length
  "Here is the longest sentence: #{word}. It is #{words} words long."
end

paragraph = "This is a test. This sentence is going to be the very longest! Here."

p longest_sentence(paragraph) #=> 0

book = "The Project Gutenberg eBook of Frankenstein; Or, The Modern Prometheus
